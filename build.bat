...
REM // make sure we can write to the file snibuilt.bin
REM // also make a backup to snibuilt.prev.bin
IF NOT EXIST snibuilt.bin goto LABLNOCOPY
IF EXIST snibuilt.prev.bin del snibuilt.prev.bin
IF EXIST snibuilt.prev.bin goto LABLNOCOPY
move /Y snibuilt.bin snibuilt.prev.bin > NUL
IF EXIST snibuilt.bin goto LABLERROR3
REM IF EXIST snibuilt.prev.bin copy /Y snibuilt.prev.bin snibuilt.bin
:LABLNOCOPY
...
"win32/s2p2bin" %s2p2bin_args% s2.p snibuilt.bin s2.h

REM // fix some pointers and things that are impossible to fix from the assembler without un-splitting their data
IF EXIST snibuilt.bin "win32/fixpointer" s2.h snibuilt.bin   off_3A294 MapRUnc_Sonic $21 0 4   word_728C_user Obj5F_MapUnc_7240 2 2 1  

REM REM // fix the rom header (checksum)
IF EXIST snibuilt.bin "win32/fixheader" snibuilt.bin
...
IF EXIST snibuilt.bin (
   convsym s2.lst snibuilt.bin -input as_lst -a
   exit /b
)
...
:LABLERROR3
echo Failed to build because write access to snibuilt.bin was denied.
pause
